    ...
    facedetector, GstVideoFrame * frame)
{
  ...
  if(!facedetector->priv->pFaceRectSeq || facedetector->priv->pFaceRectSeq->total == 0){
    ...
    std::string overlay = "/opt/prope.bmp";
  ...
  for (i = 0;
    ...
    CvRect *r;
    GstStructure *data;
    std::string overlay = std::string("/opt/sydney.bmp")
    r = (CvRect *) cvGetSeqElem (facedetector->priv->pFaceRectSeq, i);
    data = createData(r, facedetector->priv->resize_factor, getMillisecondsTime(), overlay);
